...
def _texto_indica_pneu(grupo, descricao, peca=None) -> bool:
    valores = [grupo, descricao]
    if peca is not None:
        valores.extend([peca.grupo, peca.descricao, peca.codigo])
...
def eh_item_pneu(item: ItemOS) -> bool:
    ...
    return _texto_indica_pneu(item.grupo, item.descricao, item.peca)
