...
from services.calculos import (baixar_item_os, desvincular_movimentos, devolver_item_os,
                               ...
                               validar_km)
from services.correcoes_os import _texto_indica_pneu
...
def adicionar_item(os_id):
    ...
    try:
        ...
        # Pneu precisa de posição e número de fogo individuais. Uma linha só
        # com quantidade 3, por exemplo, não dá pra abrir 3 lançamentos
        # separados — por isso quebramos em uma linha por pneu (quantidade 1
        # cada), preservando os mesmos dados nas demais.
        eh_pneu = _texto_indica_pneu(item.grupo, item.descricao, peca)
        qtd_total = item.quantidade or 1
        unidades = int(qtd_total) if eh_pneu and qtd_total and qtd_total > 1 else 1

        itens_criados = []
        for _ in range(unidades):
            novo = item if not itens_criados else ItemOS(
                ordem_servico_id=ordem.id, peca_id=item.peca_id,
                descricao=item.descricao, grupo=item.grupo,
                valor_unitario=item.valor_unitario)
            novo.quantidade = 1 if unidades > 1 else qtd_total
            db.session.add(novo)
            db.session.flush()
            baixar_item_os(novo)          # dá baixa no estoque na hora
            registrar_log("criar", "itens_os", novo.id, f"OS {ordem.numero}")
            itens_criados.append(novo)
        db.session.commit()
