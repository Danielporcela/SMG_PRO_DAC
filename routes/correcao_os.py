...
from services.correcoes_os import aplicar_posicao_pneu, eh_item_pneu, posicoes_do_veiculo
...
def definir_posicao(item_id):
    ...
    try:
        resultado = aplicar_posicao_pneu(item, posicao)
...
(() => {
  ...
  function estilo() {
    ...
    s.textContent = `
      ...
      .sgmf-item{border:1px solid #ddd;border-radius:12px;padding:14px;margin:10px 0}.sgmf-linha{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:end}
      .sgmf-item select{width:100%;padding:10px;border:1px solid #bbb;border-radius:8px}.sgmf-item button{padding:10px 14px;border:0;border-radius:8px;background:#0f766e;color:#fff;font-weight:700;cursor:pointer}
      .sgmf-fechar{float:right;border:0;background:transparent;font-size:26px;cursor:pointer}.sgmf-ok{font-size:13px;color:#166534;margin-top:8px}.sgmf-erro{font-size:13px;color:#b91c1c;margin-top:8px}
    `;
  ...
  function abrirModal() {
    ...
    modal.innerHTML = `<button class="sgmf-fechar" type="button">×</button><h3>Posição do pneu na OS</h3><p>Escolha a posição em que o pneu lançado será aplicado. O pneu que estava em uso nessa posição ficará preservado no histórico como descartado.</p>`;
    ...
    pendentes.forEach(reg => {
      ...
      const titulo = document.createElement('strong'); titulo.textContent = `${reg.ordem_numero || 'OS'} · ${reg.veiculo || 'Veículo'} · ${reg.descricao || 'Pneu'}`; box.appendChild(titulo);
      const linha = document.createElement('div'); linha.className = 'sgmf-linha';
      const sel = document.createElement('select');
      sel.innerHTML = '<option value="">Selecione a posição</option>' + (reg.posicoes || []).map(p => {
        const atual = p.ocupada ? ` · atual ${p.pneu_atual || ''}${p.sulco_mm != null ? ` · ${p.sulco_mm} mm` : ''}` : '';
        return `<option value="${p.valor.replaceAll('"','&quot;')}">${p.valor}${atual}</option>`;
      }).join('');
      ...
      aplicar.onclick = async () => {
        ...
        try {
          const r = await fetchOriginal(`/api/correcao_os/pneu/${reg.item_id}/posicao`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({posicao:sel.value})});
          const d = await r.json();
          if (!r.ok) throw new Error(d.erro || 'Falha ao registrar a posição.');
          msg.className='sgmf-ok'; msg.textContent = d.resultado?.pneu_substituido ? `Posição registrada. Pneu ${d.resultado.pneu_substituido} retirado e mantido no histórico.` : 'Posição registrada com sucesso.';
          await carregarPendentes(false);
          setTimeout(() => { box.remove(); if (!pendentes.length) bg.remove(); }, 650);
        } catch (e) { msg.className='sgmf-erro'; msg.textContent=e.message || String(e); }
        finally { aplicar.disabled=false; }
      };
      linha.append(sel, aplicar); box.append(linha, msg); modal.appendChild(box);
    });
  ...
  estilo();
  document.addEventListener('DOMContentLoaded', () => { carregarPendentes(false); recalcular(); });
  if (document.readyState !== 'loading') { carregarPendentes(false); recalcular(); }
